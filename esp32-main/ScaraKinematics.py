...
class ScaraKinematics(Parameter):
    ...
    def __init__(self, theta_length:float, phi_length:float, max_segment_size: float, right_handed: bool, **k):
        ...
        self.work_offset = dict(x=0, y=0, z=0, a=0, b=0, c=0)
    ...
    def set_phi_len(self, phi_len):
        self.phi_len = phi_len
        self.phi_2 = phi_len**2
    ...
    def translate(self, pos: CartesianPosition, work_offset=None):
        # rotate
        # print(f'f{pos = }')
        hyp = (pos['x']**2 + pos['y']**2) ** .5 # math.hypot(x, y)
        hyp_angle = math.atan2(pos['y'], pos['x'])
        if work_offset:
            new_hyp_angle = hyp_angle + work_offset['a']
        else:
            new_hyp_angle = hyp_angle + self.work_offset['a']
        ...
        if work_offset:
            for axis, position in pos.items():
                pos[axis] = position + work_offset[axis]
        else:
            for axis, position in pos.items():
