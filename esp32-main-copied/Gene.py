...
class Gene(Parameter):
    ...
    def make_sleepytime(self, seconds):
        ...
        yield {'cmd': 'sleepyhack'}  # gene expects something to return, return garbage then
    ...
    def subscribe(self, sub: tuple[str|int, str]):
        print('subbing', sub)
        if isinstance(sub, Parameter):
            if self.local_sub is not sub:    
                sub.add_hot(self)
                self.local_sub = sub
        elif sub is not self.subscription:  # are we already subscribed?
            self.iris.subscribe(header=sub[0], pid=self.pid, bundle=sub[1])
            self.subscription = sub
        
    def unsubscribe(self):
        if self.local_sub:
            self.local_sub.remove_hot(self)
            self.local_sub = None
        else:
