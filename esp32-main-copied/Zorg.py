...
class Zorg:
    ...
    def _post(self):
        msg = {'cmd': 'term', 'state': self.posts}
        self.posts = ""
        return msg
    
    def _devices(self):
        return {'cmd': 'devices', 'state': self.devices}
    
    def _files(self):
        return {'cmd': 'files', 'state': os.listdir()}
    
    def _cluster(self):
        return {'cmd': 'cluster', 'state': [[board.name, board.adr] for board in namespace.namespace.values()]}
    ...
    def _test(self):
        return {'cmd': 'my_id', 'state': self.iris.id}
